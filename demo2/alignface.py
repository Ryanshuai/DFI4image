...
def load_face_detector(predictor_path='models/shape_predictor_68_face_landmarks.dat'):
  ...
  predictor=dlib.shape_predictor(predictor_path)  #返回人脸关键点
  return detector,predictor

def detect_landmarks(ipath,detector,predictor,upsample=0,image=None):
  if image is None:
    original255=skimage.io.imread(ipath).astype(numpy.ubyte) #按照路径打开图片
    original=original255/255.0 #归一化
  else:
    original=image
    original255=(original.clip(0,1)*255).round().astype(numpy.ubyte)
  dets=detector(original255,upsample) #检测人脸
  if len(dets)!=1: raise FitError('{}: detected zero or more than one face.'.format(ipath)) #确保只检测到一张人脸

  for k,d in enumerate(dets):
    shape=predictor(original255,d) #关键点检测
    X=numpy.array([[shape.part(i).y,shape.part(i).x] for i in range(68)]).astype(numpy.float64) #输出68个关键点
  return X,original  #返回检测的关键点，和归一化的人脸图片
